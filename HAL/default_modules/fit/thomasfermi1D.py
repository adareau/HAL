...
Comments : implements a 1D Thomas Fermi fit
"""
# %% IMPORTS
...
def ThomasFermi1D(x, *p):
    return np.where(
        abs((x - p[3]) / p[2]) < 1,
        p[0] + p[1] * (1 - [(x - p[3]) / p[2]] ** 2) ** 2,
        p[0],
    )


# %% CLASS DEFINITION
class ThomasFermi1DFit(Abstract1DFit):
    """a 1D Thomas Fermi fit"""

    def __init__(self, **kwargs):
        super().__init__(**kwargs)

        # -- attributes specific to 2D Gauss fit
        self.name = "Thomas Fermi parabola"
        self.short_name = self.name
        self.category = "math"
        self.formula_help = "f(x) = p[0] "
        self.formula_help += "+ p[1] * (1- [(x - p[3])/p[2]] ** 2)**2"
        self.formula_help += "* door(|(x - p[3]))/p[2] |< 1)"
        self.parameters_help = "p = [offset, amplitude, size, center]"
        self._version = "1.0"

    def _fitfunc(self, x, *p):
        return ThomasFermi1D(x, *p)
